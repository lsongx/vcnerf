...
def load_colmap_data(realdir):
  ...
  return poses, pts3d, perm, hwf_cxcy, sorted_name

def save_poses(basedir, poses, pts3d, perm, hwf_cxcy, sorted_name):
  pts_arr = []
  vis_arr = []
  for k in pts3d:
  ...
  with open(os.path.join(basedir, 'sorted_names.txt')) as f:
    f.write('\n'.join(sorted_name))
...
def colmapGenPoses(dpath):
  ...
  if need_run_coolmap(dpath):
    ...
    runner(dpath)
    print( 'Post-colmap')
    poses, pts3d, perm, hwf_cxcy, sorted_name = load_colmap_data(dpath)
    save_poses(dpath, poses, pts3d, perm, hwf_cxcy, sorted_name)
